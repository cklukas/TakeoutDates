...
void printHelp() {
    std::cout << "Usage: google_photos_date_setter <folder> [options]\n"
              ...
              << "  --remove-named-tags \"tag\" Remove specific Finder Tags (macOS only, semicolon-separated)\n";
}
...
/**
 * Sets Finder Tags on a file (macOS only).
...
/**
 * Removes all Finder Tags from a file (macOS only).
 * @param filePath The path to the file.
 * @return True if successful, false otherwise.
 */
...
/**
 * Removes specific Finder Tags from a file (macOS only).
 * @param filePath The path to the file.
 * @param tagsToRemove The list of tags to remove.
 * @return True if successful, false otherwise.
 */
bool removeNamedFinderTags(const std::string &filePath, const std::vector<std::string> &tagsToRemove) {
    ...
    [url getResourceValue:&currentTags forKey:NSURLTagNamesKey error:&error];
    if (error) {
 ...
 * For --list, includes people names in CSV output with semicolon separator.
 ...
 * @param assignPeopleTags If true, assigns specified tags.
 * @param peopleTagsToAssign Tags to assign (if assignPeopleTags is true).
 * @param assignAllPeopleTags If true, assigns all people names as tags.
 * @param removeAllTags If true, removes all tags.
 * @param removeNamedTags If true, removes specified tags.
...
void processFile(const fs::path &jsonPath, bool listOnly, bool setDates, bool listTags,
    ...
    if (listOnly) {
        // List the primary file with people names
 ...
 * Supports date setting, tag listing, and tag management.
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--help") {
            ...
            while (std::getline(ss, tag, ';')) {
                if (!tag.empty()) peopleTagsToAssign.push_back(tag);
            }
            ...
            std::string tag;
            while (std::getline(ss, tag, ';')) {
                if (!tag.empty()) tagsToRemove.push_back(tag);
            }
        } else {
